...
from phone_target import PhoneTargetUnavailable, dispatch_release_once  # noqa: E402
...
def _observe_composite(*, serial: str, binding_key: str, admitted: object, materialized: object) -> tuple[object, object]:
    ...
    return require_observed_pair(snapshot)
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit("hosted admitted Release envelope is invalid") from exc
    ...
    if args.recovery_only != "true" and (not serial or not binding_key):
        state = reduce_state(
            _base_state_authorized(),
            "observation_refused",
